...
while stk.size() > 0:
    rm = stk.pop()
    rm_id = rm.id
    if rm_id not in grph.vertices:
        grph.add_vertex(rm_id)
    rm_connected = rm.get_exits()
    for drt in rm_connected:
        rm_connected = rm.get_room_in_direction(drt)
        rm_connected_id = rm_connected.id
        if rm_connected_id not in grph.vertices:
            grph.add_vertex(rm_connected_id)
        grph.add_edge(rm_id, rm_connected_id, drt)
...
def bfs_search_rooms(rm_id, visited, g=grph):
    rms_moves = {}
    ...
    while q.size() > 0:
        ...
        if len(adj_keys) == 1 and adj[adj_keys[0]] not in visited:
            not_explored_path_de = list(moves) + [adj_keys[0]]
...
### Combine the DFT and BFS for adventure ###
